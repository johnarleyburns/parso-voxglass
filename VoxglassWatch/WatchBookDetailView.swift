...
struct WatchBookDetailView: View {
    ...
    var body: some View {
        List {
            ...
            Section("Chapters") { ForEach(book.chapters, id: \.id) { chapter in Button { services.play(book, chapterIndex: chapter.index); showNowPlaying = true } label: { Text(chapter.title) }.accessibilityIdentifier("watch.chapter.\(chapter.id.rawValue)") } }
