...
| Offline listening and download state | Downloading is supported and book state exposes `Downloaded` | Keep download actions contextual. Add a clearly labeled **Downloaded** refinement to My Books when the library filter work is next; this is the only competitive gap worth prioritizing for the main shelf. |
