...
public struct StudioURLSessionFetcher: HTTPFetching {
    ...
    public func get(_ url: URL, timeout: TimeInterval, userAgent: String) async throws -> HTTPFetchResult {
        ...
        do {
            ...
            return HTTPFetchResult(data: data, statusCode: http.statusCode, finalURL: http.url ?? url)
...
/// Observes redirects so a hop toward a sign-in page is reported as an auth
/// wall (L3 → nothing).
