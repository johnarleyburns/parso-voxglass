-- Captured DDL snapshot of schema version 1 (initial_production_schema).
-- Rule §7.4(4): migration tests must build the OLD schema from a captured
-- snapshot, not from the current migration list, so later migrations are
-- tested against the schema real v1 installs had.
-- Keep in sync with ProductionMigration.schemaV1; SchemaMigrationTests asserts
-- this file produces an identical schema.
