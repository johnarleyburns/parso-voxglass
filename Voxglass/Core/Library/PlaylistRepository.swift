...
public final class PlaylistRepository {
    ...
    public func create(title: String) async throws -> Playlist {
    ...
    public func fetchBooks(for playlistID: UUID) async throws -> [BookWithChapters] {
        try await database.prepare()
    ...
    public func addBook(_ bookID: UUID, to playlistID: UUID) async throws {
        try await database.prepare()
    ...
    public func removeBook(_ bookID: UUID, from playlistID: UUID) async throws {
        try await database.prepare()
        try await database.execute("""
    ...
    public func rename(_ playlistID: UUID, to title: String) async throws {
        try await database.prepare()
        try await database.execute("""
    ...
    public func delete(_ playlistID: UUID) async throws {
        try await database.prepare()
    ...
    public func reorder(_ playlistID: UUID, bookIDs: [UUID]) async throws {
        try await database.prepare()
...
private enum LibraryPlaylistBook {
    public static func book(from row: DatabaseRow) throws -> BookWithChapters {
        let book = Book(
